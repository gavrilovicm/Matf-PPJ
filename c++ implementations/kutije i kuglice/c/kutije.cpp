...
void Kutija::podigni(){
	double s=0;
	for (auto i=m_koef.begin(); i!=m_koef.end(); i++)
		s += *i;

	if (m_ima_podskup) {
		for (auto i=m_podskup.begin(); i!=m_podskup.end(); i++)
			s += *i;
	}
	std::cout << "Teska je: " << s << std::endl;
}
...
void Kutija::proveri(const double& n){
	auto finder = std::find(m_koef.begin(), m_koef.end(), n);
	if (finder == m_koef.end()){
		if (m_ima_podskup) {
			auto fin = std::find(m_podskup.begin(), m_podskup.end(), n);
			if (fin == m_podskup.end())
