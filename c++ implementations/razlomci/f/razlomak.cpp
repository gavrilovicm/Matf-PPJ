...
int Razlomak::nom(){
	return this->getBrojilac();
}

int Razlomak::den(){
	// slobodna interpretacija
	// ako je razlomak nula vrati za imenioca jedinicu
	if (this->getBrojilac() == 0)
		return 1;
	else
		return this->getImenilac();
}
bool Razlomak::operator == (Razlomak& drugi){
	return ((this->getImenilac() == drugi.getImenilac())&&(this->getBrojilac() == drugi.getBrojilac()));
}

int Razlomak::getBrojilac() const {
	return this->m_brojilac;
}

int Razlomak::getImenilac() const {
