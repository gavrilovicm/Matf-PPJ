...
%token disp_tok size_tok km_tok kp_tok un_trans
%token <br> num
%token <s> id
...
E 
	...
	| id un_trans {
		...
		int** mat = prom[k].mat;
		int vrste = prom[k].br_vrsta;
		int kolone = prom[k].br_kolona;
		$$ = transponuj(mat, vrste, kolone);
		tr_vrste = kolone;
		tr_kolone = vrste;
	...
	| id '*' id {
		...
		tr_vrste = vrste_druge;
		tr_kolone = kolone_prve;
		$$ = mnozenje(mat1, mat2, kolone_prve, vrste_prve, kolone_druge, vrste_druge);
